...
def get_frq(terms, length=10, **kwargs):
    frq = {}
...
def get_mcq(terms, length=10, n_options=4, **kwargs):
    mcq = {}
    terms_copy = deepcopy(terms)
    for i in range(length):
        possible_terms = list(terms_copy.keys())
...
def get_true_false(terms, length=10, **kwargs):
    true_false = {}
    terms_copy = deepcopy(terms)
    for i in range(length):
        possible_terms = list(terms_copy.keys())
...
def get_quiz(terms, types=["mcq", "frq", "tf"], length=10, n_options=4):
    quiz_types = {
        "mcq": get_mcq,
        "frq": get_frq,
        "tf": get_true_false,
    ...
    for i in range(length):
        ...
        question = get_question(terms, length=1, n_options=n_options)
